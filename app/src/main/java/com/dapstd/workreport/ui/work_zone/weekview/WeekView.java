...
import static com.dapstd.workreport.ui.work_zone.weekview.WeekViewUtil.isSameDay;
...
public class WeekView extends View {
    ...
    private int mSelectedBackgroundColor = Color.rgb(251, 235, 254);
    ...
    private int mTodayHeaderTextColor = Color.rgb(245, 245, 245);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(getNumberOfVisibleDays()==1)drawTimeColumnAndAxes(canvas);
    }


    private void drawTimeColumnAndAxes(Canvas canvas) {
        // Draw the background color for the header column.
        /*canvas.drawRect(0,  mHeaderRowPadding * 2, mHeaderColumnWidth, getHeight(), mHeaderColumnBackgroundPaint);

        // Clip to paint in left column only.
        canvas.clipRect(0,  mHeaderRowPadding * 2, mHeaderColumnWidth, getHeight(), Region.Op.REPLACE);
        */
        for (int i = 1; i < 24; i++) {
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        canvas.clipRect(mHeaderColumnWidth, +mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2, getWidth(), getHeight(), Region.Op.REPLACE);
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mSelectedDay != null) isSelectedDay = isSameDay(day, mSelectedDay);
            ...
            if (mWidthPerDay + startPixel - start > 0) {
                if (isSelectedDay&&getNumberOfVisibleDays()>1) {
                    canvas.drawRect(start, +mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom, startPixel + mWidthPerDay, getHeight(), mSelectedBackgroundPaint);
                } else {
                    canvas.drawRect(start, +mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom, startPixel + mWidthPerDay, getHeight(), mDayBackgroundPaint);
                }
            }
    ...
    private Calendar getTimeFromPoint(float x, float y) {
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mWidthPerDay + startPixel - start > 0 && x > start && x < startPixel + mWidthPerDay) {
                Calendar day = today();
                day.add(Calendar.DATE, dayNumber - 1);
                float pixelsFromZero = y - mCurrentOrigin.y - -mHeaderRowPadding * 2 - mTimeTextHeight / 2 - mHeaderMarginBottom;
            ...
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()) {
                    ...
                    if (left < right &&
                            ...
                            bottom > mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom
                    ) {
     ...
     * </p>
     *
     * @param length Supported values are {@link com.dapstd.workreport.ui.work_zone.weekview.WeekView#LENGTH_SHORT} and
     *               {@link com.dapstd.workreport.ui.work_zone.weekview.WeekView#LENGTH_LONG}.
     */
    @Deprecated
    ...
    public void setSelectedDay(Calendar mSelectedDay) {
    ...
    public void goToDate(Calendar date) {
        ...
        long dateDifference = (dateInMillis / day) - (todayInMillis / day) - (date.getTime().getDay() + 6) % 7;
        if(getNumberOfVisibleDays()==1){
            dateDifference = (dateInMillis / day) - (todayInMillis / day);
        }
    ...
    public void goToDate(LocalDate date) {
        ...
        goToDate(new GregorianCalendar(year, month - 1, day));
    }
    ...
    public interface EventLongPressListener {
        /**
         * Similar to {@link com.dapstd.workreport.ui.work_zone.weekview.WeekView.EventClickListener} but with a long press.
         *
         * @param event:     event clicked.
         * @param eventRect: view containing the clicked event.
         */
    ...
    public interface EmptyViewLongPressListener {
        /**
         * Similar to {@link com.dapstd.workreport.ui.work_zone.weekview.WeekView.EmptyViewClickListener} but with long press.
         *
